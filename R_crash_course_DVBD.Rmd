...
#In the above line of code, we created a new data.frame object called "simdat.new". We used the assignment operator "<-" to specify what we want this new data.frame to be. This new dataframe object would be built by taking simdat data.frame AND THEN adding three new columns to it: state, bmi, and logbmi. The code following the state=, bmi=, and logmi= arguments specify what should populate these new columns. 
...
#Note that if we attempted to create this new data.frame using a new line of mutate() for each column:
simdat.new <- simdat %>% 
  mutate(state="Colorado")
simdat.new <- simdat %>% 
  mutate(bmi=wt_kg/(ht_cm/100))
simdat.new <- simdat %>% 
  mutate(logbmi=log10(wt_kg/(ht_cm/100)))
...
outcome.summ <- simdat.new %>% # To make a new data.frame called outcome.summ, first take simdat.new AND THEN...
  group_by(hospital, outcome) %>% # ... group together all of its data based on outcome AND THEN...
